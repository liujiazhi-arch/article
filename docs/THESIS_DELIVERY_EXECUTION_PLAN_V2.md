...
> 日期：2026-06-17
> 对应战略：`docs/THESIS_PRODUCT_EXPERIENCE_STRATEGY.md`（原始战略）+ `docs/THESIS_RESILIENT_DELIVERY_STRATEGY.md`（不确定性下的可信交付修订）
> 整合来源：`docs/THESIS_DELIVERY_EXECUTION_PLAN.md`（用户执行稿）+ Claude 构思稿
> 目标：把"可信论文交付工作台"拆成可执行、可验证、代码最简的阶段计划。本文件是唯一实施蓝图，战略文档不动。

---

## 0. 整合前的一个决定性事实（必须先读）

两份执行稿都假设 `delivery_reports.py` 要从 job payload 里捞结构化 findings 来渲染报告。**核对代码后这个假设是错的，而且纠正后方案更简单。**

- 持久化的 apply `result` 里，`verification` 已被裁剪成 `{overall_status, readiness}`（`job_payloads.py` build_result_summary），**不含逐条 evidence**。若走"解析 payload"路线，P0 根本拼不出学生报告。
...
1. **下载路由对 role 不做白名单。** `GET /jobs/{job_id}/artifacts/{artifact_role}/download`（routes_jobs.py:52）→ `get_job_artifact` 按 `artifact.get("role") == role` 字符串匹配。**新增 `student_report / ai_report / technical_report / failure_report` 角色无需改路由，注册即可下载。**
2. **前后端契约不枚举 role。** `tests/test_frontend_backend_contract.py` 只禁止学生界面出现内部术语（queued/running/failed/bbox/rule_id 等），不限制 role 取值。新增 role 不会撞契约，但前端文案必须为新 role 提供学生用词。

另核实：`ensure_report_artifact()` 已在 `jobs.py` 的多个 finalize 点被调用（133/262/663/689/782/836），成功与失败路径都经过它——这就是新报告挂载的天然接缝。
...
def build_delivery_reports(
    *,
    ...
    workspace: dict[str, str] | None, # 含 staged input / written output 路径
    resolved_request: dict,           # 含 profile / scopes
...
- 成功 apply/normalize：对 `workspace["output"]`（修复稿）调用 `workflow.build_audit_human_reports(...)`，把它写出的 `结论报告.md / ai_review_context.md / audit_report.md` 映射为 `student_report / ai_report / technical_report` 三个 artifact。报告反映"修复后还剩什么"，正是学生要的。
...
- 在已有调用 `ensure_report_artifact` 的 finalize 点，**紧随其后**调用 `ensure_delivery_artifacts`，成功与失败走同一入口。
- 不做历史 job backfill（避免一次性迁移代码）。历史任务若缺新报告，P1 在打开结果页时按需重新生成（on-demand），不持久化迁移。
...
- 战略文档保留：`THESIS_PRODUCT_EXPERIENCE_STRATEGY.md`、`THESIS_RESILIENT_DELIVERY_STRATEGY.md`。
- 本执行文档保留：`THESIS_DELIVERY_EXECUTION_PLAN_V2.md`。
