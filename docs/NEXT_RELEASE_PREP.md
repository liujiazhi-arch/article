...
- Python package version: `pyproject.toml` 中的 `0.1.0`
- Runtime service version: `scripts/article_api/response_payloads.py` 中的 `SERVICE_VERSION = "0.1.0"`
- GitHub Release 草稿: `docs/NEXT_GITHUB_RELEASE_DRAFT.md`
- Release 模板: `docs/GITHUB_RELEASE_TEMPLATE.md`
- 发布检查清单: `docs/RELEASE_CHECKLIST.md`
- Windows 包名: `article-local-windows.zip`
- Windows sha256: `article-local-windows.zip.sha256`
...
4. `python scripts\build_windows_local_bundle.py --runtime-dir dist\windows-runtime --output-zip dist\article-local-windows.zip --release-api-url <release-api-url>`
5. `python scripts\verify_release_artifact.py dist\article-local-windows.zip --sha256-output dist\article-local-windows.zip.sha256`
6. `python scripts\windows_bundle_smoke.py dist\article-local-windows.zip --work-dir dist\windows-bundle-http-smoke --command-timeout-seconds 600 --json-output dist\windows-bundle-smoke.json`

Release published 事件会把 `article-local-windows.zip` 和 `article-local-windows.zip.sha256` 上传到对应 GitHub Release。
...
- `CHANGELOG.md` 仍处于 `Unreleased` 状态；最终发布前应把对应条目归入实际 tag。
- Python 包版本和服务版本目前都还是 `0.1.0`；如果新版本 tag 不是 `v0.1.0-beta`，需要决定是否同步 bump。
- Windows clean 环境、WPS/Word 打开修复稿、SmartScreen 来源确认不能由 macOS 本地命令替代。

## 发布后验证

```bash
python3 scripts/github_release_status.py --repo liujiazhi-arch/article --branch main --tag <release-tag> --json-output /tmp/article-github-status.json
