...
def test_render_verify_module_script_can_show_help_from_repo_root():
    ...
    assert "--pdf-matches-docx-confirmed" in result.stdout
    assert "--generate-static-toc" in result.stdout


def test_render_verify_cli_forwards_pdf_docx_confirmation(monkeypatch, capsys):
    captured = {}
    ...
    monkeypatch.setattr(render_verify_module, "build_render_verify_report", fake_build)
    monkeypatch.setattr(render_verify_module, "render_render_verify_report", lambda report: "ok")

    exit_code = render_verify_module.main(
        [
            "demo.docx",
            "--rendered-pdf",
            "demo.pdf",
...
def test_word_pdf_export_script_targets_opened_file_not_active_document(monkeypatch, tmp_path):
    ...
    assert "close every document" not in captured["script"]
    assert "candidateName is targetName" not in captured["script"]
    assert "already open" in captured["script"]
    assert "did not expose the opened DOCX" in captured["script"]
    assert "save as docRef" in captured["script"]
    assert "close docRef saving no" in captured["script"]
    assert captured["command"][3:] == [str(source_path.resolve()), str(output_pdf.resolve())]
...
def test_external_rendered_pdf_uses_neutral_manual_pdf_evidence_source(monkeypatch, tmp_path):
    ...
    assert trust["trust"] == "unverified"
    assert trust["is_authoritative"] is False
    assert trust["layout_decision_eligible"] is False
    assert any("当前 DOCX" in warning and "确认" in warning for warning in trust["warnings"])
...
def test_evidence_trust_only_upgrades_user_confirmed_manual_pdf(
    ...
    assert trust["is_authoritative"] is expected_authoritative
    assert trust["layout_decision_eligible"] is expected_eligible
...
@pytest.mark.parametrize(
    ...
    [
        ("manual-pdf", False, "structure-ready", True, 2, 0, 0, "unsupported-evidence"),
        ("manual-pdf", True, "structure-ready", True, 2, 0, 0, "render-evidence-ready"),
        ("manual-pdf", True, "render-check-required", True, 2, 0, 0, "render-review-required"),
        ("manual-pdf", True, "structure-ready", False, 0, 0, 0, "render-review-required"),
        ("manual-pdf", True, "structure-ready", True, 2, 1, 0, "render-review-required"),
        ("manual-pdf", True, "structure-ready", True, 2, 0, 1, "render-review-required"),
        ("word-pdf", False, "structure-ready", True, 2, 0, 0, "render-evidence-ready"),
        ("wps-manual-images", True, "structure-ready", True, 2, 0, 0, "unsupported-evidence"),
    ],
)
def test_render_evidence_status_requires_eligible_complete_clean_evidence(
    evidence_source,
    confirmed,
    structure_readiness,
    text_available,
    ...
    status = render_verify_module._classify_render_evidence_status(
        ...
        verification={"readiness": structure_readiness},
        render_summary={"finding_count": finding_count},
        render_text_summary={
            "available": text_available,
            "page_text_available_count": text_page_count,
            "page_text_extraction_warning_count": text_warning_count,
            "warnings": ["warning"] if text_warning_count else [],
        },
...
def test_build_render_verify_report_accepts_external_page_images(monkeypatch, tmp_path):
    ...
    assert report["evidence_trust"] == "unverified"
    assert report["evidence_authoritative"] is False
    assert report["layout_decision_eligible"] is False
    assert report["pdf_matches_docx_confirmed"] is False
    assert report["summary"]["pdf_matches_docx_confirmed"] is False
    assert any("当前 DOCX" in warning and "确认" in warning for warning in report["render_warnings"])
    assert report["render_page_dir"] == str(page_dir.resolve())
    assert report["page_images"] == [str((page_dir / "page-1.png").resolve())]
    assert "PDF 与 DOCX 对应关系已确认: 否" in Path(report["report_path"]).read_text(encoding="utf-8")


def test_page_images_reject_pdf_docx_confirmation():
    with pytest.raises(ValueError, match="确认.*PDF"):
        render_verify_module.build_render_verify_report(
            "demo.docx",
            page_images_dir="pages",
...
def test_render_render_verify_report_includes_review_summary():
    report = {
        ...
        "toc_finalization": {
            ...
            "entry_count": 26,
            "mapped_count": 26,
            "output_path": "/tmp/render-proof/static_toc.docx",
            "message": "静态目录版已生成",
    ...
    assert "页面渲染完整度: 82/100 (检测扣分 18)" in rendered
    ...
    assert "目录终验：" in rendered
    assert "- 状态: 静态目录版已生成" in rendered
    assert "- 映射数量: 已对应 26 项 共 26 项" in rendered
    assert "- 输出文件: /tmp/render-proof/static_toc.docx" in rendered
    assert "- 说明: 静态目录版已生成" in rendered
    assert "- 下一步: 下载后重新导出 PDF 并再次复核" in rendered
    assert "internal-only-reason" not in rendered
    assert "internal-only-detail" not in rendered
