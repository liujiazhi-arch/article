...
def test_fix_roundtrip_clears_all_issues(tmp_docx, tmp_path, rule_id):
    ...
    before = audit_rule_status(source_path, rule_id, profile_path="cn-common")
    assert not before["rule"]["passed"]

    fixed_path = Path(tmp_path) / f"{rule_id.lower()}_fixed.docx"
    fix_thesis.fix_docx(str(source_path), str(fixed_path), profile_path="cn-common")

    after = audit_rule_status(fixed_path, rule_id, profile_path="cn-common")
